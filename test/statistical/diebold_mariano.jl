@testset "diebold_mariano" begin
    ...
    @testset "dm_mean" begin
        ...
        @testset "basic test" begin
            expected = 0.5
            result = mean(dm_mean_test.(eachslice(randn(rng, 10000, 10000), dims=2)))
        ...
        @testset "detects signficant mean differences" begin
            result = map([0, 0.001, 0.1, 1.0, 10.0]) do m
                data = rand(rng, Normal(m, 1), 10000)
                dm_mean_test(data)
            end
        ...
        @testset "detects significance over certain sample size" begin
            result = map([100, 500, 1_000, 5_000, 10_000]) do s
                data = rand(rng, Normal(0.1, 1), s)
                dm_mean_test(data)
            end

            @test sort(result, rev=true) == result
            @test (result .< 0.05) == [false, true, true, true, true]
        end
        ...
        @testset "p-value decays beyond critical bandwidth" begin
            ...
            s = collect(0.01:0.01:1) + rand(rng, Normal(0, 1), 100);
            x1 = s[1:end-10]
            x2 = s[11:end]
        ...
        @testset "spectral density is negative" begin
            @test_throws ArgumentError dm_mean_test(rand(rng, 100); bandwidth=80)
        end
    ...
    @testset "dm_median" begin
        ...
        @testset "basic test" begin
            ...
            data = eachslice(randn(rng, 1000, 1000); dims=2)
            expected = 0.5
